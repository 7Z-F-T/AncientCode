...
public class Tac {
	public enum Kind {
		...
		LES, ASSIGN, LOAD_VTBL, INDIRECT_CALL, DIRECT_CALL, RETURN, JUMP,
		JZERO, LOAD, STORE, LOAD_IMM4, LOAD_STR_CONST, MEMO, MARK, PARM
	}
	...
	public static Tac genJump(Label label) {
		label.target = true;
		return new Tac(Kind.JUMP, label);
	}

	public static Tac genJZero(Temp cond, Label label) {
		label.target = true;
		return new Tac(Kind.JZERO, cond, label);
	}
	...
	public String toString() {
		...
		case JZERO:
			return "if (" + op0.name + " == 0) jump " + label.name;
		case LOAD:
			if (op2.value >= 0) {
