...
import java.util.*;
/**
 * 程序主框架。其中包含了大多数程序运行时所需要的参数、属性变量，同时绘制主窗口
 * 界面，并存储所有绘制的图形对象。
 * 
 * @author 侯杰
 *
...
public class MainFrame extends JFrame{
	...
	/**
	 * 所有经绘制操作的产生的图形对象的备份，便于Undo操作时提取
	 */
	//backShapeProArray
	public ArrayList<ArrayList<ShapePro>> backShapeProArrayArray;
	
	/**
	 * 所有经Undo操作产生的图形对象的备份，便于Redo时提取
	 */
	//frontShapeProArray
	...
	/**
	 * 主画布。在其上可以作画并进行编辑修改操作
	 */
	//canvas
	public MyCanvas canvas;
	
	/**
	 * 绘图板变化拨动开关。当主程序需要通过网络发送绘图数据时，拨动此开关通知发送线程
	 */
	//flips
	public Flip repaintFlip=new Flip(1);//关注图像发生更改
	/**
	 * 聊天信息变化拨动开关。当主程序需要通过网络发送聊天数据时，拨动此开关通知发送线程
	 */
