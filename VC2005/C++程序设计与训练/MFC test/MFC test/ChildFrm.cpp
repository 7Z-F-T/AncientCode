...
BOOL CChildFrame::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
	...
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CLeftView), CSize(100, 100), pContext) ||
		!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CMFCtestView), CSize(100, 100), pContext))
	{
...
// CChildFrame 消息处理程序
CMFCtestView* CChildFrame::GetRightPane()
{
	CWnd* pWnd = m_wndSplitter.GetPane(0, 1);
	CMFCtestView* pView = DYNAMIC_DOWNCAST(CMFCtestView, pWnd);
	return pView;
}
...
void CChildFrame::OnUpdateViewStyles(CCmdUI* pCmdUI)
{
	...
	CMFCtestView* pView = GetRightPane(); 
...
void CChildFrame::OnViewStyle(UINT nCommandID)
{
	...
	// 如果右窗格已创建并且是 CMFCtestView，则处理菜单命令...
	if (pView != NULL)
	{
		int nStyle = -1;
