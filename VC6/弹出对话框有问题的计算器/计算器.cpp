...
char Dis[10]={'a','a','a','a','a','a','a','a','a','a'};
char DisOpr[5];
char *pchar;
char *stop="a";
int decimal,sign;
...
int WINAPI WinMain(HINSTANCE	hInstance,HINSTANCE hPrevInst,LPSTR lpszCmdLine,int	nCmdShow)
{
	...
	MSG Msg ;
	WNDCLASS wndclass ;
	char lpszClassName[] = "main";			//窗口类名
	char lpszTitle[]= "科学计算器——期中大作业 结52 侯杰 2005010130";			//窗口标题名

	
	//窗口类的定义
	wndclass.style = CS_HREDRAW|CS_VREDRAW|CS_DBLCLKS;						//窗口类型为缺省类型
	...
	wndclass.lpszMenuName ="mymenu" ;			//窗口中无菜单
...
LRESULT CALLBACK WndProc(HWND hwnd,UINT message,WPARAM  wParam,LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
	    	case IDM_HELP:
				MessageBox(hwnd,"这是使用说明（没写完呢）","说明",MB_OK);
				break;
			case IDM_ABOUT:
				MessageBox(NULL,"2005年5月侯杰设计","关于",MB_OK|MB_APPLMODAL);
				break;
			case IDM_EXIT:
				PostQuitMessage(0);
				break;
	...
	case WM_DESTROY:
		PostQuitMessage(0);		
		break;                          //调用PostQuitMessage发出WM_QUIT消息
