...
for m = 1:length(axisX)
    ...
    axisY(m) = M(j)*(x(j+1)-axisX(m))^3/(6*h(j)) + M(j+1)*(axisX(m)-x(j))^3/(6*h(j)) + (y(j)-M(j)*h(j)^2/6)*(x(j+1)-axisX(m))/h(j) + (y(j+1)-M(j+1)*h(j)^2/6)*(axisX(m)-x(j))/h(j);
end
...
for k =1:5
    ...
    value(k) = -M(j)*(x(j+1)-pos)^2/(2*h(j)) + M(j+1)*(pos-x(j))^2/(2*h(j)) + (y(j+1)-y(j))/h(j) - (M(j+1)-M(j))/6*h(j);
end
