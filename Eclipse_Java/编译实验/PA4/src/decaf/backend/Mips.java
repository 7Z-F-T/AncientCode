...
public class Mips implements MachineDescription {
	...
	private RegisterAllocater regAllocater;
	...
	public Mips() {
		...
		regAllocater = new RegisterAllocater(fpTemp, frameManager, GENERAL_REGS);
		stringConst = new TreeMap<String, String>();
	}

	@Override
	public void emitTac(List<FlowGraph> gs) {
		emit(null, ".text", null);
		for (FlowGraph g : gs) {
			regAllocater.reset();
			Iterator<BasicBlock> iter = g.iterator();
			while (iter.hasNext()) {
				iter.next().label = Label.createLabel();
			}
			iter = g.iterator();
			while (iter.hasNext()) {
	...
	private void genAsmForBB(BasicBlock bb) {
		for (Tac tac = bb.tacChain; tac != null; tac = tac.next) {
			switch (tac.opc) {
			case ADD:
	...
	private void emitTrace(BasicBlock bb, FlowGraph graph) {
		...
		case BY_JUMP:
			...
			break;
		case BY_JZERO:
			emit(null, String.format(MipsAsm.FORMAT2, "beqz", bb.var.reg, graph
					.getBlock(bb.next[0]).label.name), null);
