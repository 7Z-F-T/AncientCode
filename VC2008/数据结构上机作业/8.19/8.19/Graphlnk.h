...
//	void DFS(const T& v);					//Depth First Search
//	void BFS(const T& v);					//Breath First Search
private:
...
bool Graphlnk<T,E>::insertVertex(const T vertex)
{
	...
	{
		cerr<<"The table of vertexex has full!"<<endl;
		return false;
	}
...
{
	if(v <0 || v>= maxVertices)	{cerr<<"Invalid vertex ID!"<<endl;return false;}
	if(curVertices == 1)
	{cerr<<"This Vertex is the last one vertex in the graph!"<<endl;return false;}
	Edge<T,E> *p,*s,*t;int k;
	while(NodeTable[v].adj != NULL)				//Delete all nodes int the vth adjacent list
	{
		p = NodeTable[v].adj;
		k = p->dest;
		...
		if(s != NULL)
		{
			if(t == NULL)	NodeTable[k].adj = s->link;		//s is the first item of the adjacent list(free of circulation)
			else			t->link = s->link;
			delete s;
		}
		NodeTable[v].adj = p->link;				//Delete the initial edge from the vth adjacent list
		delete p;
		curEdges--;
	}
	curVertices--;
...
void Graphlnk<T,E>::f_input(istream &in)
{
	...
	cout<<"Please input the information (vertexes and weight) of edges:"<<endl;
	i=0;
	while(i<m)
	{
		in>>e1>>e2>>weight;
		...
		if(j == -1 || k == -1 || j == k)
			cerr<<"One or more vertex error,enter aegin!"<<endl;
...
template<class T, class E>
void Graphlnk<T,E>::DFS(int v, bool visited[])			//For Reservation
{
	cout << getValue(v) << ' ';
	visited[v] = true;
