...
int main(int argc,char* argv[])
{
	...
	pthread_t tid; 
	const char sep[]=":";
	char* ip=strtok(argv[2],sep);//ip地址
	int port=atoi(strtok(NULL,sep));//端口号
	SOAP_SOCKET m, s; 
	m = soap_bind(&soap, ip, port, BACKLOG); 
	...
	if(GetReady(filename)==false) return -1;
	else
...
int SearchService__Search (struct soap *soap,string query,struct SearchService__Result &result)
{
	...
	input.GetSearchWordAmount();
	input.GetSearchWordFromString();
	if(input.nWord==0) return SOAP_OK;
	else
	{
