...
/**
 * 主画布。在主画布上完成所有的绘图操作。主画图负责所有图形的显示及更新。
 * @author 侯杰
 *
 */
public class MyCanvas extends JPanel{
	...
	float[] dashPattern={7,7,7,7,7,7,7};
	BufferedImage texture=new BufferedImage(4,4,BufferedImage.TYPE_INT_ARGB);
	/**
	 * 新建一块主画布。
	 * @param array 存储所有图形的ArrayList<ShapePro>对象
	 * @param property 当前绘图的默认属性
	 */
	...
	public void paint(Graphics g){
		...
		//画白底
		Rectangle canvasBound=new Rectangle(0,0,getWidth(),getHeight());
		g2.setColor(Color.WHITE);
		g2.fill(canvasBound);
		
		//开始输出所有图形
		for(int i=0;i<shapeProArray.size();i++){
			ShapePro s=shapeProArray.get(i);
			//如果是文字对象，先要生成shape
			...
			//设置基本绘图属性
			g2.setStroke(new BasicStroke(s.stroke.width,BasicStroke.CAP_BUTT,BasicStroke.JOIN_MITER,10.0f,s.stroke.pattern,0));
			...
			if(s.fillType==FillType.SOLID){
				...
				g2.setPaint(new GradientPaint(s.gradientFillColor.point1,s.gradientFillColor.color1,s.gradientFillColor.point2,s.gradientFillColor.color2,true));
				g2.fill(transformedShape);
			}
			//判断是否被指向，如果被指向就填充纹理
			if(s.pointed==true){
