...
public class DrawHandler implements MouseMotionListener,MouseListener{
		    ...
		    case Function.SHEAR://对于shear,采集变换参数，待将shape正变换后输出
		    	if(currentSelectedShapePro!=null&&currentSelectedShapePro.selected==true&&startInCtrlPoint==true){
		    		...
		    		if(tempY>currentSelectedShapePro.getCenterPoint().y)
		    			currentSelectedShapePro.shearedFactorX+=dx/120;
		    		else
		    			currentSelectedShapePro.shearedFactorX-=dx/120;
		    		if(tempX>currentSelectedShapePro.getCenterPoint().x)
		    			currentSelectedShapePro.shearedFactorY+=dy/120;
		    		else
		    			currentSelectedShapePro.shearedFactorY-=dy/120;
		    		System.out.println("facX:"+currentSelectedShapePro.shearedFactorX+" facY"+currentSelectedShapePro.scaledFactorY);
		    		lastX=e.getX();
		    		lastY=e.getY();
		    		canvas.repaint();
		    		break;
			...
			case Function.ELLIPSE:
			 ...
			 default:
				...
				if(flag==true){//鼠标点击在了图形s的区域内
					...
					if(currentSelectedShapePro!=null){
						//System.out.println("Deselect");
						if(inCtrlPoint(e.getX(),e.getY())==false)
						currentSelectedShapePro.selected=false;//取消上次的选择
						canvas.repaint();
					}
				}
				if(inCtrlPoint(e.getX(),e.getY())==true)
    ...
    public boolean inCtrlPoint(double mouseX, double mouseY){
    	Point2D.Double p=new Point2D.Double(mouseX,mouseY);
    	return (ShapePro.c1.contains(p)||ShapePro.c2.contains(p)||
			    ShapePro.c3.contains(p)||ShapePro.c4.contains(p));
    }
}
